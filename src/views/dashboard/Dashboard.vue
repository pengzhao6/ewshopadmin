<template>
  <div class="content px-4">
    <div class="mt-4 bg-white">
      <div>
        <n-card
            style="border: 1px solid #bed0ea;text-align: left"
            title="系统信息"
            :segmented="{
            content: true,
            footer: 'soft'
  ...
  <div class="flex">
    <div style="width: 48%" class="mt-4 bg-white ml-5">
      ...
      >
          <div class="goods">
            ...
            <div>
              <div>下架商品</div><div><span>{{ goods_info.recommend_nums }}</span>件</div>
            </div>
            <div>
              <div>缺货商品</div><div><span>{{ goods_info.stock_null }}</span>件</div>
            </div>
            <div>
              <div>推荐商品</div><div><span>{{ goods_info.un_nums }}</span>件</div>
            </div>
          </div>
      </n-card>
    </div>
    ...
    <div style="width: 48%" class="mt-4 bg-white ml-5">
      ...
      >
          <div class="goods">
            <div>
              <div>今日访客</div><div><span>{{ users_info.today_nums }}</span>件</div>
            </div>
            <div>
              <div>昨日访客</div><div><span>{{ users_info.yesterday_nums }}</span>件</div>
            </div>
            <div>
              <div>累计访客</div><div><span>{{ users_info.total_nums }}</span>件</div>
            </div>
            <div>
              <div>累计注册</div><div><span>{{ users_info.reg_nums }}</span>件</div>
            </div>
        </div>
      </n-card>
    </div>
...
<style lang="less" scoped>
  ...
  .goods > div {
    ...
    height: 50px;
    line-height: 50px;
    border-bottom:1px dashed #bed0ea
  }
